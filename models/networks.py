...
def init_net(net, init_type="normal", init_gain=0.02, gpu_ids=None,
    ...
    Parameters:
        ...
        gpu_ids (list)     -- legacy GPU IDs (ignored, DDP handles device placement)
        debug (bool)       -- CUT compat (ignored)
        initialize_weights (bool) -- whether to apply weight initialization
...
def define_G(input_nc, output_nc, ngf, netG, norm="batch", use_dropout=False, init_type="normal", init_gain=0.02,
             gpu_ids=[], no_antialias=True, no_antialias_up=True, opt=None):
    """Create a generator
    ...
    Parameters:
        ...
        netG (str) -- the architecture's name: resnet_9blocks | resnet_6blocks | unet_128 | unet_256 | spif_9blocks | spif_6blocks | spif_dual_9blocks | spif_dual_6blocks
        norm (str) -- the name of normalization layers used in the network: batch | instance | none
        use_dropout (bool) -- if use dropout layers.
        init_type (str)    -- the name of our initialization method.
        init_gain (float)  -- scaling factor for normal, xavier and orthogonal.
        gpu_ids (int list) -- which GPUs the network runs on (legacy, for CUT/UTOM compat)
        no_antialias (bool) -- if True, skip antialiased downsampling (CUT)
        no_antialias_up (bool) -- if True, skip antialiased upsampling (CUT)
        opt (option class) -- the full option object (needed by CUT)
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm="batch", init_type="normal", init_gain=0.02,
             no_antialias=True, gpu_ids=[], opt=None):
    ...
    Parameters:
        ...
        init_gain (float)  -- scaling factor for normal, xavier and orthogonal.
        no_antialias (bool) -- CUT antialias flag
        gpu_ids (int list) -- legacy compat
        opt (option class) -- full option object
...
def define_F(input_nc, netF, norm="batch", use_dropout=False, init_type="normal", init_gain=0.02,
    ...
    """Create a feature projection network for PatchNCE loss (CUT).

    Parameters:
        input_nc (int) -- number of input channels
        netF (str) -- projection type: mlp_sample | sample | reshape | global_pool | strided_conv
        norm (str) -- normalization type
        use_dropout (bool) -- if use dropout
        init_type (str) -- initialization method
        init_gain (float) -- scaling factor
        no_antialias (bool) -- CUT antialias flag
        gpu_ids (int list) -- GPUs
        opt (option class) -- full option object (needed for netF_nc)

    Returns a feature projection network
    """
    if netF == "global_pool":
        net = PoolingF()
    elif netF == "reshape":
...
class ResnetGenerator(nn.Module):
    """Resnet-based generator (compatible with CUT feature extraction mode).

    Standard forward: `forward(input)` returns the generated image.
    Feature extraction: `forward(input, layers=[...], encode_only=True)`
    returns intermediate features for PatchNCE loss (used by CUT).
    """

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,
                 use_dropout=False, n_blocks=6, padding_type="reflect",
                 no_antialias=True, no_antialias_up=True, opt=None):
        assert n_blocks >= 0
    ...
    def forward(self, input, layers=[], encode_only=False):
        """Forward pass with optional feature extraction for CUT/NCE loss."""
        if -1 in layers:
...
class SPIFGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,
        ...
        # Deeper encoder heads: 7×7 conv + downsampling for each modality
        ...
        model += [
            nn.ConvTranspose2d(ngf * 2, ngf, kernel_size=3, stride=2, padding=1,
                               output_padding=1, bias=use_bias),
            norm_layer(ngf),
            nn.ReLU(True),
        ]
...
class SPIFDualGenerator(nn.Module):
    ...
    its own dedicated output projection.

    Only supports output_nc == 2 (one channel per modality).
    """
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,
        ...
        # Shared backbone (same as ResnetGenerator minus the output layer)
        model = [nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(ngf), nn.ReLU(True)]
    ...
    def forward(self, x):
        ...
        bf = self.head_bf(feat)
        af = self.head_af(feat)
        return torch.cat([bf, af], dim=1)
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if no_antialias:
                ...
                sequence += [nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
                             norm_layer(ndf * nf_mult), nn.LeakyReLU(0.2, True), Downsample(ndf * nf_mult)]
...
##############################################################################
# CUT / Contrastive Learning support classes (merged from CUT repo)
##############################################################################
